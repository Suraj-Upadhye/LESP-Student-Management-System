...
export class ViewAttendanceComponent {
  ...
  attendanceDataArrayAssigned: any[] = [];
  attendanceData: any = {
    data: {
      "2024-04-06": {
        attendance: [
          { rollNo: "233207", status: "Present", studentName: "Shubham Dhondiram Sargar" },
          { rollNo: "233208", status: "Present", studentName: "Suraj Shantinath Upadhye" },
          { rollNo: "233207", status: "Absent", studentName: "Shubham Dhondiram Sargar" },
          { rollNo: "233208", status: "Present", studentName: "Suraj Shantinath Upadhye" },
          { rollNo: "233207", status: "Present", studentName: "Shubham Dhondiram Sargar" },
          { rollNo: "233208", status: "Absent", studentName: "Suraj Shantinath Upadhye" }
        ],
        batch: "",
        ...
        semester: "5",
        sessionType: "Lecture",
        subjectName: "CLIENT-SIDE SCRIPTING LANGUAGE",
        year: "TY"
      }
    },
    success: true
  ...
  transformAttendanceData(): any[] {
    const attendanceDataArray: any[] = [];
    for (const attendanceObj of this.attendanceData.data.attendanceData) {
      const date = attendanceObj.date;
      const sessionType = attendanceObj.sessionType;
      const attendanceRecords: any[] = attendanceObj.studentList;
      attendanceRecords.forEach((record: any, index: number) => {
        const attendanceItem = {
          ...
          date: index === 0 ? date : '',
          sessionType: index === 0 ? sessionType : '',
          totalLecture: attendanceRecords.length,
          statusArray: Array(attendanceRecords.length).fill(''),
          totalPresent: 0,
          totalAbsent: 0,
          percentage: ''
        };
        attendanceItem.statusArray[index] = record.state;
        if (record.state === 'Present') {
          attendanceItem.totalPresent++;
        } else if (record.state === 'Absent') {
          attendanceItem.totalAbsent++;
        }
        attendanceItem.percentage = `${((attendanceItem.totalPresent / attendanceItem.totalLecture) * 100).toFixed(2)}%`;
        attendanceDataArray.push(attendanceItem);
      });
    }
    this.attendanceDataArrayAssigned = attendanceDataArray;
    ...
    this.adminService
      ...
      .subscribe({
        next: (res) => {
          ...
          const data = this.transformAttendanceData()
          console.log(data)
